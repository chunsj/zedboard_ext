...
static void read_section_ptr(U2_T Form, ELF_Section ** Section, U8_T * Offs, ELF_Section * DefSection) {
...
static void read_object_info(U2_T Tag, U2_T Attr, U2_T Form) {
    ...
    if (Tag == TAG_compile_unit || Tag == TAG_partial_unit || Tag == TAG_type_unit) {
        ...
        case AT_comp_dir:
            ...
            break;
        case AT_stmt_list:
            read_section_ptr(Form, &Unit->mLineInfoSection, &Unit->mLineInfoOffs, sUnitDesc.mVersion <= 1 ? sCache->mDebugLineV1 : sCache->mDebugLineV2);
            break;
        case AT_base_types:
        ...
        case AT_language:
            ...
            break;
        case AT_addr_base:
            read_section_ptr(Form, &Unit->mAddrInfoSection, &Unit->mAddrInfoOffs, sCache->mDebugAddr);
            break;
        case AT_loclists_base:
            read_section_ptr(Form, &Unit->mLocListsSection, &Unit->mLocListsOffs, sCache->mDebugLocLists);
            break;
        case AT_rnglists_base:
            read_section_ptr(Form, &Unit->mRngListsSection, &Unit->mRngListsOffs, sCache->mDebugRngLists);
            break;
...
U8_T read_dwarf_addr_section(CompUnit * Unit, U4_T idx, ELF_Section ** Section) {
    ...
    U8_T offs = idx * Unit->mDesc.mAddressSize * 2 + Unit->mAddrInfoOffs;
    DWARFCache * Cache = (DWARFCache *)Unit->mFile->dwarf_dt_cache;
    if (Cache->mDebugAddr == NULL) str_exception(ERR_INV_DWARF, "Missing .debug_addr section");
    dio_EnterSection(&Unit->mDesc, Cache->mDebugAddr, offs);
...
static void load_debug_sections(void) {
    ...
    for (idx = 1; idx < file->section_cnt; idx++) {
        ...
        else if (strcmp(sec->name, ".debug_addr") == 0) {
            sCache->mDebugAddr = sec;
        }
        else if (strcmp(sec->name, ".debug_str_offsets") == 0) {
            sCache->mDebugStrOffsets = sec;
        }
